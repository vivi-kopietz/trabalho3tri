criaCartão(
    'Matematica',
    'Como se calcula a soma dos termos de uma P.A',
    'Utiliza-se a fórmula: ((a1+an)*r)/2'
)

criaCartão(
    'História',
    'Quanto tempo durou a Ditadura Civil-Militar no Brasil?',
    '21 anos'
)

criaCartão(
    'Português',
    'O que é um verbo?',
    'É uma classe gramatical que expressa ações, estados, fenômenos naturais, localizando-os no tempo'
)

criaCartão(
    'Artes',
    'que cor se obtem na mistura das cores branco e vermelho?',
    'cor rosa'
)

criaCartão(
    'Biologia',
    'Quantos ossos tem o corpo humano adulto? ',
    '206 ossos'
)

criaCartão(
    'Geografia',
    'Quantos continentes existem e quais são eles?',
    'Existem 6 continentes sendo eles America, Europa, Africa, Asia, Antartica e Oceania'
)

criaCartão(
    'Física',
    'O que é MUV',
    'Movimento Uniformemente Variado- é um movimento em que a velocidade varia de forma constante a cada segundo '
)

criaCartão(
    'Química',
    'O que são hidrocarbonetos',
    'Compostos binários de Hidrogênio e Carbono'
)

criaCartão(
    'Educação Financeira',
    'Quanto de impostos o Brasil cobra por produtos importados',
    'O Brasil cobra 20% para produtos abaixo de U$50, e acima 60%'
)

criaCartão(
    'Empreendedorismo',
    'O que é um perfil empreendedor?',
    'É uma soma de atitudes e caracteristica que ajudam o profissional em sua trajetória'
)

criaCartão(
    'Geografia',
    'A Inglaterra esta localizada na Ásia ou na Europa?',
    ' Na europa'
)

criaCartão(
    'História',
    'Em que ano a escravidão foi abolida no Brasil?',
    '1888'
)

criaCartão(
    'Filosofia',
    'Qual o filosofo responsável pelo Mito da Caverna?',
    'Platão'
)

criaCartão(
    'Geografia',
    'qual o maior país do mundo?',
    'Rússia'
)

criaCartão(
    'Artes',
    'Quem pintou a Noite Estrelada ',
    'Vicent Van Gogh'
)

criaCartão(
    'Programação',
    'Como criar um link para o seu projeto?',
    'importe seu projeto no aplicativo versel'
)

criaCartão(
    'biologia',
    'Quais são as fases do desenvolvimento embrionario?',
    'Zigoto, Morula, blastocisto e glastrula'
)

criaCartão(
    'Química',
    'qual o prefixo correspondente a 4 carbonos?',
    'But'
)

criaCartão(
    'Educação Fisíca',
    'Que ano foi criado o punhobol?',
    '1870'
)

criaCartão(
    'História',
    'quanto tempo durou a era Vargas??',
    'durou 15 anos'
)